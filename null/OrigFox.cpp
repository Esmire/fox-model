...
Calls check overlap in a loop.*/
bool OrigFox::placeFoxOnMap(int n, std::vector<OrigFox>* pop, int xSize, int ySize, bool wrongFormula) { //use std vector + make clearer
    (*pop)[n].genPos(xSize, ySize); //emplace_back also cast bool to int
    (*pop)[n].genRadius();
    double overlap;
    double geoMean = 0;
    int i = 0;
    bool tooLarge = false;
    while (i < n && !tooLarge) { //Pushback
...
void OrigFox::genPos(int xSize, int ySize) {
    ...
    double randCompare = (double)(rand() % 100 + 1) / 100.0; //Analogue to "keep" variable in R
